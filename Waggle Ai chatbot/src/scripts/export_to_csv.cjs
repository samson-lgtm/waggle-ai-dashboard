...
function formatJanToStandard(line) {
  if (!line || line.trim().startsWith('Channel')) return null;
...
try {
  ...
  if (fs.existsSync(janPath)) {
    ...
    console.log('Jan data processed: ' + (finalRows.length - 1) + ' rows.');
  }

  const janRowsCount = finalRows.length - 1;
  ...
  if (fs.existsSync(febPath)) {
    ...
    console.log('Feb data processed: ' + febRows.length + ' rows.');
  }
  ...
  if (fs.existsSync(marPath)) {
    ...
    console.log('Mar data processed: ' + marRows.length + ' rows.');
  }
